#
class CRC16(object):
    crc16_tab_h = {0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0,
    ...
    crc16_tab_l = {0x00, 0xC0, 0xC1, 0x01, 0xC3, 0x03, 0x02, 0xC2, 0xC6, 0x06,
                   0x07, 0xC7, 0x05, 0xC5, 0xC4, 0x04, 0xCC, 0x0C, 0x0D, 0xCD,
                   0x0F, 0xCF, 0xCE, 0x0E, 0x0A, 0xCA, 0xCB, 0x0B, 0xC9, 0x09,
                   0x08, 0xC8, 0xD8, 0x18, 0x19, 0xD9, 0x1B, 0xDB, 0xDA, 0x1A,
                   0x1E, 0xDE, 0xDF, 0x1F, 0xDD, 0x1D, 0x1C, 0xDC, 0x14, 0xD4,
                   0xD5, 0x15, 0xD7, 0x17, 0x16, 0xD6, 0xD2, 0x12,
    ...
    def calculate_checksum(self, data):
        return self.calculate_checksum(data, 0, len(data))

    def calculate_checksum(self, data, offset, length):
        return self.calculate_checksum(data, offset, length, 0xffff)

    def calculate_checksum(self, data, offset, length, preval):
        ucCRCHi = (preval & 0xff00) >> 8
        ucCRCLo = preval & 0x00ff
        for i in range(length):
            iIndex = (ucCRCLo ^ data[offset + i] & 0x00ff)
            ucCRCLo = ucCRCHi ^ self.crc16_tab_h[iIndex]
            ucCRCHi = self.crc16_tab_l[iIndex]
